...
def colunas(objeto: list) -> int:
    """
    @param objeto: list -> Recebe uma lista de objetos
    """
    total_letras = 0
...
def draw_table(n_colunas: int, objeto: list, space: int = 2) -> None:
    """
    @param n_colunas: int -> Recebe o numero de colunas a construir
    @param objeto: list -> Recebe uma lista de objetos
    """
    total_colunas = 0
    ...
    if (col > lin):
        caracteres = col + space
    else:
        caracteres = lin + space

    # Defini a quantidade de caracteres para dividir as linhas
    divisoria = ((n_colunas - 1) * caracteres + (n_colunas))
    ...
    for linha in range(n_colunas):
        ...
        if (total_colunas >= n_colunas):
            ...
            title = titulo_coluna
            print(
                f"|{title[(total_colunas-1)].center(caracteres, " ").replace("_", " ").title()}", end="")
    print("—" * divisoria)
        ...
        if (total_linhas >= total_items_obj):
            ...
            for value in item_value:
                if isinstance(value, float):
                    ...
                    print(f"|{str(value).center(
                        caracteres, " ").title()}", end="")
            print("", end="|")
            print("")
            print("—" * divisoria)
...
def tabela_vertical(numero_colunas: int, lista_objeto: list):
    ...
    # Pega o numero maior de caracteres das chaves
    caracteres_colunas = colunas(lista_objeto)
    # Paga o numero maior de caracteres dos valores
    caracteres_linhas = linhas(lista_objeto)
    caracteres = 0
